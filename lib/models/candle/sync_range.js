 ...
 * @param {object} candleMarket - market definition
 * @param {string} candleMarket.exchange - exchange ID
 * @param {string} candleMarket.type - data type
 * @param {string} candleMarket.symbol - symbol
 * @param {string} candleMarket.tf - timeframe
 * @param {object} range - range
 * @param {number} range.start - start timestamp
 * @param {number} range.end - end timestamp
 * @param {Function} [onSyncStart] - called with sync ranges before REST
 *   queries
 * @param {Function} [onSyncEnd] - called with sync ranges after REST queries
 * @returns {Promise} p
 */
const syncCandlesRange = async (candleModel, {
  ...
  return PI.mapSeries(consolidatedBounds, async ({ start, end }) => (
    ...
    return onSyncEnd(consolidatedBounds)
  }).catch((e) => {
...
module.exports = syncCandlesRange
